...
# 兩步驟：標題 + 截圖
step1_imgs = [
    ("1. 點「儲存到記事」", "images/2026-04-28_21-31-30.png", ACCENT),
    ("2. 工作室 → 匯出至文件", "images/2026-04-28_21-34-20.png", ACCENT3),
]
for i, (title, img_path, color) in enumerate(step1_imgs):
    x = Inches(0.8 + i * 6.0)
    add_text(slide, x, Inches(2.5), Inches(5.7), Inches(0.5),
             title, size=18, bold=True, color=color, font=FONT_TITLE)
    if os.path.exists(img_path):
        slide.shapes.add_picture(
            img_path, x, Inches(3.1),
            width=Inches(5.7), height=Inches(3.0))

add_callout(slide, Inches(0.8), Inches(6.4), Inches(11.5), Inches(0.7),
...
接下來的順序：函數本質 → 學測落點例子 → 28×28 手寫辨識例子 → 總結表 → 接龍函數 → 輪盤/溫度 → Token → 幻覺 → 觀察重點 → 不該丟給 AI。""")


# AI = 函數（基本概念）
slide = content_slide(None, "AI 的本質是「函數」", ACCENT5)
# 左欄：文字
add_text(slide, Inches(0.8), Inches(1.9), Inches(6.5), Inches(0.5),
         "國中數學：輸入 → 規則 → 輸出",
         size=20, color=LIGHT_GRAY)
add_callout(slide, Inches(0.8), Inches(2.7), Inches(6.5), Inches(1.2),
            "華氏 = 1.8 × 攝氏 + 32",
            color=ACCENT, size=20)
add_text(slide, Inches(0.8), Inches(4.2), Inches(6.5), Inches(0.5),
         "寫成 y = f(x)——黑盒子有兩個參數",
         size=16, color=LIGHT_GRAY)
add_text(slide, Inches(0.8), Inches(4.85), Inches(6.5), Inches(0.5),
         "AI 做的事情一模一樣，只是函數複雜得多",
         size=16, color=LIGHT_GRAY)

# 右欄：函數流程示意圖
if os.path.exists("img/04-function-flow.png"):
    slide.shapes.add_picture(
        "img/04-function-flow.png", Inches(7.5), Inches(1.9),
        width=Inches(5.0), height=Inches(3.4))
...
add_text(slide, Inches(0.8), Inches(1.85), Inches(11.5), Inches(0.5),
         ...
         size=18, color=LIGHT_GRAY)
# 左：簡化函數
add_callout(slide, Inches(0.8), Inches(2.6), Inches(6.5), Inches(2.8),
            "f(x) = 國文 × 0.3\n"
            "        + 英文 × 0.25\n"
            "        + 數學 × 0.35\n"
            "        + 社會 × 0.05\n"
            ...
            color=ACCENT4, size=15)
# 右：學測落點截圖
if os.path.exists("images/2026-04-29_11-28-33.png"):
    slide.shapes.add_picture(
        "images/2026-04-29_11-28-33.png", Inches(7.6), Inches(2.6),
        width=Inches(5.0), height=Inches(2.8))

add_text(slide, Inches(0.8), Inches(5.6), Inches(11.5), Inches(0.4),
...
add_text(slide, Inches(0.8), Inches(1.85), Inches(11.5), Inches(0.5),
         ...
         size=20, color=LIGHT_GRAY)
# 左：黑板示意
if os.path.exists("img/06-blackboard.png"):
    slide.shapes.add_picture(
        "img/06-blackboard.png", Inches(0.8), Inches(2.5),
        width=Inches(6.0), height=Inches(3.6))
# 右：接龍機率示意（兩張疊放）
if os.path.exists("img/06-prob-step1.png"):
    slide.shapes.add_picture(
        "img/06-prob-step1.png", Inches(7.1), Inches(2.5),
        width=Inches(5.5), height=Inches(1.7))
if os.path.exists("img/06-prob-step2.png"):
    slide.shapes.add_picture(
        "img/06-prob-step2.png", Inches(7.1), Inches(4.3),
        width=Inches(5.5), height=Inches(1.7))

add_callout(slide, Inches(0.8), Inches(6.3), Inches(11.5), Inches(0.7),
            "就像 LINE 鍵盤建議下一個字——AI 做一樣的事，只是猜得非常準",
            color=ACCENT, size=15)
...
add_text(slide, Inches(0.8), Inches(1.9), Inches(6.5), Inches(0.5),
         "AI 不是每次都選機率最高的字",
         size=20, bold=True, color=WHITE, font=FONT_TITLE)
add_text(slide, Inches(0.8), Inches(2.55), Inches(6.5), Inches(0.5),
         "像轉「輪盤」——機率高的容易中，不是一定",
         size=16, color=LIGHT_GRAY)
add_callout(slide, Inches(0.8), Inches(3.4), Inches(6.5), Inches(2.5),
            "溫度低 → 回答比較固定\n\n"
            "溫度高 → 回答有創意，比較容易出錯",
            color=ACCENT4, label="🌡  溫度參數控制隨機性",
            size=15)
# 右：輪盤圖
if os.path.exists("img/06-roulette.png"):
    slide.shapes.add_picture(
        "img/06-roulette.png", Inches(7.6), Inches(2.0),
